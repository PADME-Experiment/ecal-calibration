	...
	if (icalled == 0) {
		...
		if(ifirst_connect== 0) {
			...
			if (nRetCode == ERR_NO_ERROR) {
				...
				MPI_InitialAxis(g_RobotJ1, 200, 2000, 2000, 20000, 150, 1, 2000, -1000);
				MPI_InitialAxis(g_RobotJ2, 400, 4000, 4000, 40000, 150, 2, 2000, -2000);
				// ifirst_connect = 1;
			} else {
	...
	if(iok==0) {
		...
		if (iResult > 0) {
			...
			if (strcmp(command,"goabs ")==0) {
				...
				MPI_MoveAbsolute(g_RobotJ1, realx);
				printf("Absolute moving  x to %0.3f mm...\n", realx);

				// check if y is at home pos
				// MPI_IsHomed(g_RobotJ2, &bHomey);
				MPI_MoveAbsolute(g_RobotJ2, realy);
				printf("Absolute moving  y to %0.3f mm...\n", realy);
				...
				sprintf_s(sendbuffer, 100, "absmv ; %7.3f ; %7.3f \n", realx, realy);
				// Echo the buffer back to the sender
				iSendResult = send(ClientSocket, sendbuffer, iResult, 0);
				if (iSendResult == SOCKET_ERROR) {
					printf("send failed with error: %d\n", WSAGetLastError());
			...
			if (strcmp(command, "gorel ") == 0) {
				...
				double diffx = realx;
				MPI_MoveRelative(g_RobotJ1, diffx);
				// and recalculate position
				realx = oldrealx + diffx;
				printf("Relative moving x by %0.3f to %0.3f mm...\n", diffx, realx);

				double diffy = realy;
				MPI_MoveRelative(g_RobotJ2, diffy);
				// and recalculate position
				realy = oldrealy + diffy;
				printf("Relative moving y by %0.3f to %0.3f mm...\n", diffy, realy);
				...
				sprintf_s(sendbuffer, 100, "relmv ; %7.3f ; %7.3f \n", diffx, diffy);
				// Echo the buffer back to the sender
				iSendResult = send(ClientSocket, sendbuffer, iResult, 0);
				if (iSendResult == SOCKET_ERROR) {
					printf("send failed with error: %d\n", WSAGetLastError());
			...
			if (strcmp(command, "whereis ") == 0) {
				...
				MPI_GetFeedbackPos(g_RobotJ1, &newrealx);
				MPI_GetFeedbackPos(g_RobotJ2, &newrealy);

				Sleep(2000);

				printf("whereis : %7.3f , %7.3f \n", oldrealx, oldrealy);

				sprintf_s(sendbuffer, 100, "where ; %7.3f ; %7.3f \n", oldrealx, oldrealy);
				// Echo the buffer back to the sender
				iSendResult = send(ClientSocket, sendbuffer, iResult, 0);
				if (iSendResult == SOCKET_ERROR) {
					printf("send failed with error: %d\n", WSAGetLastError());
